...
HINT:
    go beyond.
        i.e,, if level-4, reach level-5
*/
import java.util.*;
class SpendMinPoints_DP{
    // Recursive solution...

	// Plain recursive way..
	public static int minPoints(int n, int[] arr){
        if(n ==0 || n == 1)
            return arr[n];
        return arr[n] + Math.min(minPoints(n-1, arr), minPoints(n-2, arr));
    }
    ...
    public static int minPoints(int n, int[] arr){
        //if(n ==0 || n == 1)
        //    return arr[n];
        if (map.get(n) != null)     // If already in the map, use it..
            return map.get(n);
        
        // if not in map, calculate it..
        map.put(n, arr[n]+Math.min(minPoints(n-1, arr), minPoints(n-2, arr)));
    ...
    public static void main(String [] args){
        ...
        int arr[] = new int[length+1];      // Fills the array with 0, by default..
        ...
        map.put(0, arr[0]);
        map.put(1, arr[1]);
        System.out.println("Min points: "+minPoints(length, arr));
    }
}
